...
def compare_dicts(dict1, dict2):
    ...
    if isinstance(dict1, dict):
        ...
        if len(dict1) != len(dict2):
            return False
        sorted_dict1 = sorted(dict1, key=lambda x: str(x))
        sorted_dict2 = sorted(dict2, key=lambda x: str(x))
        for val1, val2 in zip(sorted_dict1, sorted_dict2):
...
dict1 = {'a': 1, 'b': [1, 2, 3, {'c': 5}]}
dict2 = {'b': [3, 2, 1, {'c': 5}], 'a': 1}
